...
# Apache 2.0.
# This script implements CTC-CRF training on aidatatang_1505 dataset.
...
dir=`pwd -P`
data=$dir/data
aidatatang_wav=/mnt/workspace/hujt/aidatatang_1505/wav
aidatatang_trans=/mnt/workspace/hujt/aidatatang_1505/
lang_dir=data/lang_phn_test
...
if [ $stage -le 1 ]; then
  ...
  local/data_prep.sh $aidatatang_wav $aidatatang_trans/aidatatang.txt || exit 1;
  ...
  local/train_lms.sh data/local/train/text data/local/dict_phn/lexicon.txt data/local/lm_phn || exit 1;
  # Compile the language-model FST and the final decoding graph TLG.fst
  local/decode_graph.sh data/local/lm_phn data/lang_phn data/lang_phn_test || exit 1;
fi
...
if [ $stage -le 10 ]; then
    ...
    python3 ctc-crf/train_dist_chunk_context.py --model='chunk_twin_context'  --tr_data_path=data/pkl/tr --dev_data_path=data/pkl/cv \
      --den_lm_fst_path=data/den_meta/den_lm.fst \
      --regmodel_checkpoint=models/best_model_reg  \
      --data_loader_workers=0  --gpu_batch_size=$gpu_batch_size --feature_size=$feature_size \
      --output_unit=$output_unit --layers=$layers --hdim=$hdim  --dropout=$dropout  --origin_lr=$origin_lr --stop_lr=$stop_lr \
      --dist_url=$dist_url --world_size=$world_size --start_rank=$start_rank  --default_chunk_size=$chunk_size --jitter_range=$jitter_range --cate=$cate  || exit 1;
