...
# Apache 2.0.
# This script implements CTC-CRF training on BigBNAH dataset.
...
stage=1
dir=`pwd -P`
BigBNAH_wav=/home/ouzj02/hjt/BigBNAH_V3/wav
BigBNAH_trans=/home/ouzj02/hjt/BigBNAH_V3/transcript
lang_dir=/home/ouzj02/hjt/graph_tlg_bigbn_new
...
if [ $stage -le 1 ]; then
  ...
  local/data_prep.sh $BigBNAH_wav $BigBNAH_trans/BigBNAH.txt || exit 1;
fi
...
if [ $stage -le 6 ]; then
  fbankdir=fbank
  for set in CNTVHNTest KinglineTest; do
    steps/make_fbank_pitch.sh --cmd "$train_cmd" --nj $nj data/$set exp/make_fbank_pitch/$set $fbankdir || exit 1;
    utils/fix_data_dir.sh data/$set || exit;
    steps/compute_cmvn_stats.sh data/$set exp/make_fbank_pitch/$set $fbankdir || exit 1;
  done
...
if [ $stage -le 10 ]; then
    ...
    python3 ctc-crf/train_dist_chunk_twin_context.py --model='chunk_twin_context'  --tr_data_path=data/pkl/tr --dev_data_path=data/pkl/cv \
      --den_lm_fst_path=data/den_meta/den_lm.fst \
